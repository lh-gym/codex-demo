...
dat <- data.frame(Y=factor(Y), age=as.numeric(age), sex=sex, smoke=smoke)
table(dat$Y)
```
...
做法：把每个变量拆成“平行项 + 趋势项”，并用 `xij` 把趋势项在第 j 个 cutpoint 乘以同一个 `fj`。

```{r}
# 构造“趋势项”变量（与原变量数值相同，但会被 xij 赋予 cutpoint 权重）
dat$age_tr   <- dat$age
dat$sex_tr   <- dat$sex
...
# 为每个趋势项构造 xij 矩阵：n 行、K 列，每列对应该 cutpoint 的乘数
XIJ <- list(
  "age_tr"   = matrix(rep(fj_linear, each = n), n, K),
  "sex_tr"   = matrix(rep(fj_linear, each = n), n, K),
  "smoke_tr" = matrix(rep(fj_linear, each = n), n, K)
)

m6 <- vglm(
  Y ~ age + sex + smoke + age_tr + sex_tr + smoke_tr,
  family = cumulative(parallel = TRUE, reverse = TRUE),   # 基项平行
  data = dat,
  xij = XIJ                                                 # 统一 f_j 作用在所有趋势项上
)
summary(m6)
```
...
* `age, sex, smoke` = **平行部分 β**（各 cutpoint 相同）
...
XIJ_varspec <- list(
  "age_tr"   = matrix(rep(fj_age,   each = n), n, K),
  "sex_tr"   = matrix(rep(fj_sex,   each = n), n, K),
  "smoke_tr" = matrix(rep(fj_smoke, each = n), n, K)
)
...
* **模型 (6)**：用 `xij` 给所有趋势项一个**共同**的 `fj`（如线性 0,1,2）
* **模型 (7)**：用 `xij` 给每个趋势项**各自**的 `fj`（变量特定约束），这就是论文里 `diag(f_j)` 的落地实现
