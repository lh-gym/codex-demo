...
m6 <- vglm(
  ...
  family = cumulative(parallel = TRUE ~ age + sex + smoke, reverse = TRUE),
  data = dat,
  constraints = constraint_linear                       # 统一 f_j 作用在所有趋势项上
)
summary(m6)
```
...
* `age, sex, smoke` = **平行部分 β**，靠 `parallel = TRUE ~ age + sex + smoke` 让三个 cutpoints 共用同一个值；
...
m7 <- vglm(
  ...
  family = cumulative(parallel = TRUE ~ age + sex + smoke, reverse = TRUE),
  data = dat,
  constraints = constraint_varspec                    # 变量特定的 f_j
)
summary(m7)
```
...
* `age, sex, smoke` = **平行部分 β**，同样借助 `parallel = TRUE ~ age + sex + smoke` 让 3 个 cutpoints 共用同一个系数；
